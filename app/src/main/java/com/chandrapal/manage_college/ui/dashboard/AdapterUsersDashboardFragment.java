...
public class AdapterUsersDashboardFragment extends RecyclerView.Adapter<AdapterUsersDashboardFragment.ViewHolderUsersDashboardFragment> {
    List<Model> dataHolder;
    onImageClickInterfaceForUsers onImageClickInterface;

    public AdapterUsersDashboardFragment(List<Model> dataHolder, onImageClickInterfaceForUsers onImageClickInterface ) {
    ...
    @NotNull
    @Override
    public ViewHolderUsersDashboardFragment onCreateViewHolder(@NonNull @NotNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.single_row_users_fragment_dashboard, parent, false);
        return new AdapterUsersDashboardFragment.ViewHolderUsersDashboardFragment(view, onImageClickInterface);
    }

    @RequiresApi(api = Build.VERSION_CODES.N)
    @Override
    public void onBindViewHolder(@NonNull @NotNull AdapterUsersDashboardFragment.ViewHolderUsersDashboardFragment holder, int position) {
...
//        Picasso.get().load(dataHolder.get(position).getPhotosImage()).into(holder.profileImage);
    ...
    public static class ViewHolderUsersDashboardFragment extends RecyclerView.ViewHolder{
        TextView uploadedTime, username, caption, password, fullname;
         ShapeableImageView profile_image;
        ShapeableImageView profileImage;
        onImageClickInterfaceForUsers onImageClickInterfaceViewHolder;

        public ViewHolderUsersDashboardFragment(@NonNull @NotNull View itemView, onImageClickInterfaceForUsers onImageClickInterface) {
            super(itemView);
...
//            profileImage = itemView.findViewById(R.id.profileImage);
            ...
            profile_image.setOnClickListener(v->onImageClickInterfaceViewHolder.onImageClickMethodForUsers(getAbsoluteAdapterPosition()));
            fullname.setOnClickListener(v->onImageClickInterfaceViewHolder.onImageClickMethodForUsers(getAbsoluteAdapterPosition()));

        }
    }

    public interface onImageClickInterfaceForUsers {
        void onImageClickMethodForUsers(int position);
    }
